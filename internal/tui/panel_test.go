...
func TestStartupHeaderRendersAndSuppresses(t *testing.T) {
	...
	on := newModel(&fakeRunner{}, meta, make(chan loop.UIEvent), nil, nil, nil, true, nil, nil, nil)
	...
	off := newModel(&fakeRunner{}, meta, make(chan loop.UIEvent), nil, nil, nil, false, nil, nil, nil)
