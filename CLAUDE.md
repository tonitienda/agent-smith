...
- **Request a Copilot review once Gemini's is fully tackled.** After every Gemini review thread is addressed and resolved (per the rule above), request a GitHub Copilot review on the PR (`request_copilot_review`). Stay subscribed and keep watching: Copilot's review comments arrive as PR activity events just like Gemini's, so investigate and tackle each one with the same reply-then-resolve discipline. Request Copilot only once per review cycle — don't re-request after pushing fixes unless a human asks for another pass.
