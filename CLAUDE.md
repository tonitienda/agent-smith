...
- **Request a Copilot review once Gemini's review is fully tackled.** Gemini Code Assist reviews PRs automatically; once its review has been posted and every Gemini review thread is addressed and resolved (per the rule above), request a GitHub Copilot review on the PR (`request_copilot_review`). (Don't treat "zero unresolved Gemini threads" as satisfied before Gemini has actually reviewed — wait for its review to land first.) Stay subscribed and keep watching: Copilot's review comments arrive as PR activity events just like Gemini's, so investigate and tackle each one with the same reply-then-resolve discipline. Request Copilot only once per review cycle — don't re-request after pushing fixes unless a human asks for another pass.
- **Auto-merge once the PR is clean.** When every review thread (Gemini, Copilot, and any human) is resolved and the work is complete, enable GitHub's native auto-merge (`enable_pr_auto_merge`) so the PR merges itself the moment required checks pass. Prefer this over a manual merge: CI *success* is not delivered as a PR activity event (only failures wake the session), so relying on the session to notice green and merge is unreliable — let GitHub do it. Use `merge_pull_request` directly only as a fallback when you are already awake and have confirmed CI is green. Never enable auto-merge while a thread is unresolved or a fix is still pending.
