...
- [x] `docs/projects/harness-quality-system.md` is reviewed and linked from `docs/agent-quality-gates.md`.
- [x] The docs define quick, full, architecture, and CI-local harness entry points and when to use each.
- [x] The docs include a local-CI parity table mapping each CI job to a local command.
- [x] Agent guidance in `CLAUDE.md`/`AGENTS.md` references the harness contract instead of only naming the full gate.
- [x] The final response format for agents remains compatible with the repository testing-summary convention.
