...
| [AS-142](AS-142-archtest-conformance-schema-guard.md) | Archtest: add layering guard for `internal/provider/conformance` and `schema` | quality | ready-to-implement | AS-098, AS-141 |
| [AS-144](AS-144-keychain-unreachable-error-classification.md) | `auth set/status` leaks a raw dbus error instead of the actionable env-var hint when no Secret Service is reachable | faces | ready-to-implement | AS-017 |
