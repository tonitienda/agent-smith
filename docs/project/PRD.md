...
**The Decision Log now lives in its own file: [decisions.md](decisions.md) (D0–D9).**
It is product truth and **read it first** — where it conflicts with sections 1–10
below, it wins. Sections 1–10 remain the original exploration; D0–D9 are the
locked decisions from a design stress-test.
...
> Several are now resolved in the Decision Log (D-series) above — kept here for the trail. Still genuinely open: Q4 (`/clean` matching engine), Q5 (ACP vs. custom protocol), Q7 (discovery/evidence). Q13 (immutability vs. erasure) is **architecture-resolved and narrowed** by the AS-056 spike ([docs/design/compliance-archiving.md](../design/compliance-archiving.md)) — crypto-shredding at the archive layer + redaction-at-capture + legal-hold, no V1 schema change; what remains is named legal questions for counsel before the paid archive ships. Q12 (plugin trust depth) is **narrowed** by the AS-059 spike ([docs/design/plugin-trust.md](../design/plugin-trust.md)) — the permission-scope vocabulary, default context-slice exclusions, declarative-only enforcement + residual risks, and the WASM-if-ever code recommendation are decided; what remains is the D9-named v1 punt (prompt-injection defense via a plugin's own model calls) and marketplace distribution/signing (§7.26). The sibling question — hosting a live agent for *strangers* (multi-tenant arbitrary code execution) — is **resolved** by the AS-080 spike ([docs/design/hosted-agent-sandboxing.md](../design/hosted-agent-sandboxing.md)): close in favour of AS-079 (read-only inspector) as the public demo; `smith serve` ships for local use only. The isolation bar required *if* hosted live execution is ever revisited is documented there but spawns no tickets now.
...
13. **Immutability vs. erasure** — *architecture resolved* by the AS-056 spike ([docs/design/compliance-archiving.md](../design/compliance-archiving.md)): the append-only invariant is *intra-session* (projection/replay soundness); erasure is an **archive-lifecycle** operation one layer up, reconciled by **crypto-shredding at the archive layer** (per-session/per-subject keys, BYO-KMS, destroy-key-to-erase) + **redaction-at-capture** (OSS, best-effort minimization — AS-115) + **legal-hold**. **No V1 block-schema change is required** (the D2 call). What remains is narrowed to named **legal** questions for counsel before the paid archive ships (controller/processor determination, crypto-shred sufficiency per regulator, sector retention windows, backup propagation) — not blocking V1 or any current backlog.
