...
**From validation report to regression fixture.** The captures this validation pass produces (AS-060) should not become one-off reports — they feed the recorded-provider regression harness. [`capture-to-fixture.md`](capture-to-fixture.md) (AS-135) documents the workflow and `cmd/capture-fixture` tool that turn a redacted, reviewed capture into a deterministic, CI-safe fixture (`docs/design/captures/`) replayed by the vendor simulators (AS-133) and the offline E2E suite (AS-134), so schema-validation pressure keeps guarding the adapters over time.
